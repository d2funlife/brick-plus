...
{
    public class ThemeSchemeToVisibility : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, string language)
        {
            var enumValue = (Theme)Enum.Parse(typeof(Theme), value.ToString());

            var parameters = parameter?.ToString().Split(',').ToArray();
            if (parameters?.Length != 2) return Visibility.Collapsed;

            var parameterValue = (Theme)Enum.Parse(typeof(Theme), parameters[0]);
